...
Definition extensional_eqMap := @ext_eq A (option B).
...
Proof.
  intros.
  apply extensional_eqMap.
  intros.
  unfold remove.
  induction (beq_A a x).
  unfold not_in_dom in H.
  rewrite <- a0 in H. auto.
  auto.
Qed.
...
Proof.
  ...
  induction (beq_A a x).
  induction (beq_A a x').
  rewrite a0 in a1. contradiction. auto.
  induction (beq_A a x'). auto. auto.
Qed.
...
Proof.
  ...
  induction (beq_A a x). auto. induction (M1 a); auto.
Qed.
...
Proof.
  ...
  unfold merge.
  induction (beq_A a x).
  rewrite <- a0 in H. unfold not_in_dom in H.
  rewrite H. auto.
  induction (M1 a); auto.
Qed.
...
Proof.
  ...
  induction (beq_A a x).
  rewrite a0. unfold not_in_dom in H1. auto. 
  auto.
Qed.
...
Proof.
  ...
  apply extensional_eqMap.
  intro.
  assert (merge M3 M2 a = merge M3 M4 a).
  rewrite <- eqMerge. auto.
  unfold merge in H.
  assert (in_dom a M3 \/ not_in_dom a M3).
  apply in_or_notin.
  inversion_clear H0.
  rewrite eq13 in disj1.
  unfold disjoint in disj1.
  unfold disjoint in disj2.
  assert (not_in_dom a M3 \/ not_in_dom a M4); auto.
  assert (not_in_dom a M3 \/ not_in_dom a M2); auto.
  inversion_clear H0.
...
Proof.
  ...
  inversion_clear ED.
  assert (in_dom a M1 \/ not_in_dom a M1).
  apply in_or_notin.
  inversion_clear H1.
  assert (in_dom a M3). apply H. auto.
  unfold in_dom in H2. inversion_clear H2.
  unfold in_dom in H1. inversion_clear H1.
  assert (merge M1 M2 a = merge M3 M4 a). rewrite MG. auto.
  unfold merge in H1.
  ...
  apply in_or_notin.
  inversion_clear H1.
  assert (in_dom a M1). apply H0. auto.
...
Proof.
  ...
  assert (M' a = None \/ exists y, M' a = Some y).
  induction (M' a). right. eauto. left. auto.
  inversion_clear H0.
  rewrite H1. auto.
  inversion_clear H1.
  rewrite H0.
  ...
  rewrite <- H0.
  assert (M' a = M a).
  apply H. unfold in_dom. exists x. auto. auto.
Qed.
...
Module HMod := MapFun (HSpec).

Definition Heap := HMod.Map.
